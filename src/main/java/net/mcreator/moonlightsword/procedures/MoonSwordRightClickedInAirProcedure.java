...
double speed = 0;double yaw = 0;
		speed = (double) 0.1;yaw = (double) (entity.rotationYaw);if ((==(false))) {if(entity instanceof PlayerEntity && !entity.world.isRemote()){
	((PlayerEntity)entity).sendStatusMessage(new StringTextComponent("FireBall Ability Activated"), (true));
}	if(world instanceof ServerWorld) {
			Entity entityToSpawn = new FireballEntity(EntityType.FIREBALL, (World) world);
		entityToSpawn.setLocationAndAngles(x, y, z, (float) 0, (float) 0);
		entityToSpawn.setRenderYawOffset((float) 0);
		entityToSpawn.setRotationYawHead((float) 0);
		entityToSpawn.setMotion((speed*Math.cos(((yaw+90)*(Math.PI/180)))),((entity.getMotion().getY())+0.2),(speed*Math.sin(((yaw+90)*(Math.PI/180)))));
...
new Object() {
	...
	((PlayerEntity)entity).sendStatusMessage(new StringTextComponent("FireBall Ability is on Cooldown"), (true));
}}
	}
}
